...
from skipper import cli, config, utils
...
SKIPPER_CONF_BUILD_CONTAINER_FQDN_IMAGE = (
    REGISTRY + "/" + SKIPPER_CONF_BUILD_CONTAINER_IMAGE + ":" + SKIPPER_CONF_BUILD_CONTAINER_TAG
)
SKIPPER_CONF = {
    "registry": REGISTRY,
...
class TestCLI(unittest.TestCase):
    ...
    def test_subcommand_help(self):
        for subcmd in ("build", "push", "make", "run"):
            result = self._invoke_cli(global_params=None, subcmd=subcmd, subcmd_params=["--help"])
            self.assertEqual(result.exit_code, 0)
    ...
    def test_build_existing_image(self, skipper_runner_run_mock):
        build_params = ["image1"]
        self._invoke_cli(global_params=self.global_params, subcmd="build", subcmd_params=build_params)
        expected_command = [
            "build",
            "--network=host",
            "--build-arg",
    ...
    @mock.patch("os.path.exists", mock.MagicMock(autospec=True, return_value=True))
    @mock.patch("skipper.runner.run", autospec=True, return_value=0)
    def test_build_existing_image_with_allow_local(self, skipper_runner_run_mock):
        global_params = self.global_params + ["--allow-local"]
        build_params = ["image1"]
        self._invoke_cli(global_params=global_params, subcmd="build", subcmd_params=build_params)
        expected_command = [
            "build",
            "--network=host",
            "--pull",
    ...
    @mock.patch(
        "skipper.utils.get_images_from_dockerfiles",
        mock.MagicMock(autospec=True, return_value={"image1": "/home/user/work/project/Dockerfile.image1"}),
    )
    @mock.patch("skipper.git.get_hash", mock.MagicMock(autospec=True, return_value="1234567"))
    @mock.patch("os.path.exists", mock.MagicMock(autospec=True, return_value=True))
    @mock.patch("skipper.runner.run", autospec=True, return_value=0)
    def test_build_existing_image_with_context(self, skipper_runner_run_mock):
        build_params = ["image1", "--container-context", "/home/user/work/project"]
        self._invoke_cli(global_params=self.global_params, subcmd="build", subcmd_params=build_params)
        expected_command = [
            "build",
            "--network=host",
    ...
    def test_build_with_context_from_config_file(self, skipper_runner_run_mock):
        build_params = ["image1"]
        self._invoke_cli(defaults=config.load_defaults(), subcmd="build", subcmd_params=build_params)
        expected_command = [
            "build",
            "--network=host",
            "--build-arg",
    ...
    @mock.patch("os.path.exists", mock.MagicMock(autospec=True, return_value=True))
    @mock.patch(
        "skipper.config.load_defaults", mock.MagicMock(autospec=True, return_value=SKIPPER_CONF_WITH_CONTEXT_NO_TAG)
    )
    @mock.patch("skipper.git.get_hash", mock.MagicMock(autospec=True, return_value="1234567"))
    ...
    def test_make_without_build_container_tag_with_context(self, skipper_runner_run_mock):
        ...
        self._invoke_cli(
            defaults=config.load_defaults(), global_params=global_params, subcmd="make", subcmd_params=make_params
        )
        expected_commands = [
            mock.call(
                [
        ...
        expected_commands = [
            ...
            mock.call(
                [
                    ...
                    "TAG=1234567",
                    "-f",
                    "/home/user/work/project/Dockerfile.image2",
                    "-t",
                    "image2:1234567",
                    "/home/user/work/project",
                ]
    ...
    @mock.patch(
        "os.path.abspath", mock.MagicMock(autospec=True, return_value="/home/user/work/project/app1/Dockerfile")
    )
    @mock.patch("os.path.exists", mock.MagicMock(autospec=True, return_value=True))
    @mock.patch(
        "skipper.config.load_defaults", mock.MagicMock(autospec=True, return_value=SKIPPER_CONF_WITH_CONTAINERS)
    )
    @mock.patch("skipper.git.get_hash", mock.MagicMock(autospec=True, return_value="1234567"))
    @mock.patch("skipper.runner.run", autospec=True, return_value=0)
    def test_build_with_defaults_from_config_file_including_containers(self, skipper_runner_run_mock):
        ...
        expected_command = [
            ...
            "TAG=1234567",
            "-f",
            "/home/user/work/project/app1/Dockerfile",
            "-t",
            "image1:1234567",
    ...
    def test_push(self, skipper_runner_run_mock, requests_get_mock):
        ...
        with mock.patch("requests.Response", autospec=True) as requests_response_class_mock:
            ...
            requests_response_mock.json.return_value = {"name": "my_image", "tags": ["latest", "aaaaaaa", "bbbbbbb"]}
            requests_get_mock.return_value = requests_response_mock

        self._invoke_cli(global_params=self.global_params, subcmd="push", subcmd_params=push_params)
        expected_commands = [
    ...
    def test_push_with_defaults_from_config_file(self, skipper_runner_run_mock, requests_get_mock):
        ...
        self._invoke_cli(defaults=config.load_defaults(), subcmd="push", subcmd_params=push_params)
        expected_commands = [
            mock.call(["tag", "my_image:1234567", "registry.io:5000/my_image:1234567"]),
            mock.call(["push", "registry.io:5000/my_image:1234567"]),
            mock.call(["rmi", "registry.io:5000/my_image:1234567"]),
    ...
    def test_images_with_single_local_results(self, subprocess_check_output_mock, tabulate_mock):
        ...
        tabulate_mock.assert_called_once_with(
            [["none", "my_image", "1234567"]], headers=["REGISTRY", "IMAGE", "TAG"], tablefmt="grid"
        )
    ...
    @mock.patch("requests.get", autospec=True)
    @mock.patch("subprocess.check_output", autospec=True)
    def test_images_with_all_results(
        self, subprocess_check_output_mock, requests_get_mock, tabulate_mock, requests_bearer_auth_mock
    ):
        ...
        self._invoke_cli(global_params=self.global_params, subcmd="images", subcmd_params=["-r"])
        ...
        requests_get_mock.assert_called_once_with(url=expected_url, auth=requests_bearer_auth_mock(), verify=False)
        ...
        tabulate_mock.assert_called_once_with(
            expected_images_results, headers=["REGISTRY", "IMAGE", "TAG"], tablefmt="grid"
        )
    ...
    def test_images_with_remote_results_only(
        self, subprocess_check_output_mock, requests_get_mock, tabulate_mock, requests_bearer_auth_mock
    ):
        with mock.patch("requests.Response", autospec=True) as requests_response_class_mock:
            requests_response_mock = requests_response_class_mock.return_value
        ...
        expected_url = "https://%(registry)s/v2/my_image/tags/list" % dict(registry=REGISTRY)
        requests_get_mock.assert_called_once_with(url=expected_url, verify=False, auth=requests_bearer_auth_mock())
    ...
    @mock.patch("requests.get", autospec=True)
    @mock.patch("subprocess.check_output", autospec=True, return_value="")
    def test_images_with_missing_remote_results(
        self, subprocess_check_output_mock, requests_get_mock, tabulate_mock, requests_bearer_auth_mock
    ):
        with mock.patch("requests.Response", autospec=True) as requests_response_class_mock:
            requests_response_mock = requests_response_class_mock.return_value
    ...
    @mock.patch("requests.get", autospec=True)
    @mock.patch("subprocess.check_output", autospec=True)
    def test_images_with_local_result_and_missing_remote_results(
        self, subprocess_check_output_mock, requests_get_mock, tabulate_mock, requests_bearer_auth_mock
    ):
        subprocess_check_output_mock.return_value = '{"name": "my_image", "tag": "aaaaaaa"}'
    ...
    @mock.patch("requests.get", autospec=True)
    @mock.patch("subprocess.check_output", autospec=True, return_value="")
    def test_images_with_with_remote_error(
        self, subprocess_check_output_mock, requests_get_mock, requests_bearer_auth_mock
    ):
        with mock.patch("requests.Response", autospec=True) as requests_response_class_mock:
            requests_response_mock = requests_response_class_mock.return_value
            requests_response_mock.ok = False
        ...
        result = self._invoke_cli(global_params=self.global_params, subcmd="images", subcmd_params=["-r"])
    ...
    def test_rmi_local(self, subprocess_check_output_mock):
        self._invoke_cli(global_params=self.global_params, subcmd="rmi", subcmd_params=["my_image", "1234567"])

        expected_command = ["docker", "rmi", "my_image:1234567"]
        subprocess_check_output_mock.assert_called_once_with(expected_command)
    ...
    def test_rmi_remote(self, requests_get_mock, requests_delete_mock, requests_bearer_auth_mock):
        ...
        self._invoke_cli(global_params=self.global_params, subcmd="rmi", subcmd_params=["-r", IMAGE, TAG])
    ...
    def test_rmi_remote_fail(self, requests_get_mock, requests_delete_mock, requests_bearer_auth_mock):
        ...
        result = self._invoke_cli(global_params=self.global_params, subcmd="rmi", subcmd_params=["-r", IMAGE, TAG])
    ...
    def test_run_with_existing_local_build_container(self, skipper_runner_run_mock):
        command = ["ls", "-l"]
        run_params = command
        self._invoke_cli(global_params=self.global_params, subcmd="run", subcmd_params=run_params)
    ...
    @mock.patch("subprocess.check_output", mock.MagicMock(autospec=True, return_value="1234567\n"))
    @mock.patch("skipper.runner.run", autospec=True)
    def test_run_with_allow_local(self, skipper_runner_run_mock):
        command = ["ls", "-l"]
        global_params = self.global_params + ["--allow-local"]
        self._invoke_cli(global_params=global_params, subcmd="run", subcmd_params=command)
        expected_image_name = "build-container-image:build-container-tag"
        skipper_runner_run_mock.assert_called_once_with(
            command,
            fqdn_image=expected_image_name,
    ...
    def test_run_with_non_existing_build_container(self, requests_get_mock):
        ...
        command = ["ls", "-l"]
        run_params = command
        ret = self._invoke_cli(global_params=self.global_params, subcmd="run", subcmd_params=run_params)
    ...
    @mock.patch("subprocess.check_output", mock.MagicMock(autospec=True, return_value="1234567\n"))
    @mock.patch("skipper.runner.run", autospec=True)
    def test_run_with_defaults_and_env_from_env_file(self, skipper_runner_run_mock):
        command = ["ls", "-l"]
        run_params = command
        self._invoke_cli(defaults=SKIPPER_CONF_WITH_ENV_FILE, subcmd="run", subcmd_params=run_params)
        expected_fqdn_image = "skipper-conf-build-container-image:skipper-conf-build-container-tag"
        skipper_runner_run_mock.assert_called_once_with(
            command,
            fqdn_image=expected_fqdn_image,
    ...
    @mock.patch("subprocess.check_output", mock.MagicMock(autospec=True, return_value="1234567\n"))
    @mock.patch("skipper.runner.run", autospec=True)
    def test_run_with_defaults_and_env_from_multiple_env_file(self, skipper_runner_run_mock):
        command = ["ls", "-l"]
        run_params = command
        self._invoke_cli(defaults=SKIPPER_CONF_WITH_MULTIPLE_ENV_FILES, subcmd="run", subcmd_params=run_params)
        expected_fqdn_image = "skipper-conf-build-container-image:skipper-conf-build-container-tag"
        skipper_runner_run_mock.assert_called_once_with(
            command,
            fqdn_image=expected_fqdn_image,
    ...
    def test_run_with_env_overriding_config_file(self, skipper_runner_run_mock):
        ...
        self._invoke_cli(defaults=SKIPPER_CONF_WITH_ENV, subcmd="run", subcmd_params=run_params)
    ...
    @mock.patch("subprocess.check_output", mock.MagicMock(autospec=True, return_value=""))
    @mock.patch(
        "skipper.utils.image_to_dockerfile", mock.MagicMock(autospec=True, side_effect=lambda x: "Dockerfile." + x)
    )
    @mock.patch("skipper.runner.run", autospec=True, return_value=0)
    def test_run_without_build_container_tag(self, skipper_runner_run_mock):
        ...
        command = ["ls", "-l"]
        run_params = command
        self._invoke_cli(global_params=global_params, subcmd="run", subcmd_params=run_params)
        expected_commands = [
            mock.call(
                [
                    "build",
    ...
    def test_run_with_publish_single_port(self, skipper_runner_run_mock):
        ...
        self._invoke_cli(global_params=global_params, subcmd="make", subcmd_params=make_params)
    ...
    def test_make(self, skipper_runner_run_mock):
        ...
        target = "all"
        make_params = ["-f", makefile, target]
        self._invoke_cli(global_params=self.global_params, subcmd="make", subcmd_params=make_params)
        expected_command = ["make", "-f", makefile, target]
        expected_fqdn_image = "build-container-image:build-container-tag"
        skipper_runner_run_mock.assert_called_once_with(
            expected_command,
    ...
    def test_make_with_additional_make_params(self, skipper_runner_run_mock):
        ...
        res = self._invoke_cli(global_params=self.global_params, subcmd="make", subcmd_params=make_params)
        self.assertEqual(res.exit_code, 0, res.exc_info)
        expected_command = ["make", "-f", "Makefile"] + make_params
        expected_fqdn_image = "build-container-image:build-container-tag"
        skipper_runner_run_mock.assert_called_once_with(
            expected_command,
            fqdn_image=expected_fqdn_image,
    ...
    def test_make_with_defaults_from_config_file(self, skipper_runner_run_mock):
        ...
        target = "all"
        make_params = ["-f", makefile, target]
        self._invoke_cli(defaults=config.load_defaults(), subcmd="make", subcmd_params=make_params)
        expected_command = ["make", "-f", makefile, target]
        expected_fqdn_image = "skipper-conf-build-container-image:skipper-conf-build-container-tag"
        skipper_runner_run_mock.assert_called_once_with(
            expected_command,
