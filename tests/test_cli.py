...
class TestCLI(unittest.TestCase):
    ...
    def test_build_multiple_images_with_invalid_image(self, skipper_runner_run_mock, *args):
        build_params = ['image1', 'image2']
        self._invoke_cli(
            global_params=self.global_params,
            subcmd='build',
    ...
    @mock.patch('skipper.runner.run', autospec=True, return_value=1)
    def test_build_multiple_images_with_non_existing_dockerfile(self, skipper_runner_run_mock, os_path_exists_mock, *args):
    ...
    @mock.patch('__builtin__.open', create=True)
    @mock.patch('os.path.abspath', autospec=True, return_value='/home/user/work/project/app1/Dockerfile')
    @mock.patch('os.path.exists', autospec=True, return_value=True)
    @mock.patch('yaml.load', autospec=True, return_value=SKIPPER_CONF_WITH_CONTAINERS)
    @mock.patch('skipper.git.get_hash', autospec=True, return_value='1234567')
    @mock.patch('skipper.runner.run', autospec=True, return_value=0)
    def test_build_with_defaults_from_config_file_including_containers(self, skipper_runner_run_mock, *args):
        build_params = ['image1']
        self._invoke_cli(
            defaults=config.load_defaults(),
            subcmd='build',
    ...
    @mock.patch('skipper.git.get_hash', autospec=True, return_value='1234567')
    @mock.patch('skipper.runner.run', autospec=True)
    def test_push_tag_fail(self, skipper_runner_run_mock, *args):
    ...
    def test_run_with_existing_local_build_container(self, skipper_runner_run_mock, *args):
        ...
        skipper_runner_run_mock.assert_called_once_with(command, fqdn_image=expected_image_name, environment=[],
                                                        interactive=False, net='host', volumes=None, workdir=None, user=None)
    ...
    def test_run_interactive_from_environment(self, skipper_runner_run_mock, *args):
        ...
        expected_fqdn_image = 'build-container-image:build-container-tag'
        skipper_runner_run_mock.assert_called_once_with(command, fqdn_image=expected_fqdn_image, environment=[],
                                                        interactive=True, net='host', volumes=None, workdir=None, user=None)
    ...
    def test_run_with_non_default_net(self, skipper_runner_run_mock, *args):
        ...
        expected_fqdn_image = 'build-container-image:build-container-tag'
        skipper_runner_run_mock.assert_called_once_with(command, fqdn_image=expected_fqdn_image, environment=[],
                                                        interactive=False, net='non-default-net', volumes=None,
                                                        workdir=None, user=None)

    @mock.patch('__builtin__.open', create=True)
    @mock.patch('os.path.exists', autospec=True, return_value=True)
    @mock.patch('yaml.load', autospec=True, return_value=SKIPPER_CONF_WITH_VOLUMES)
    @mock.patch('subprocess.check_output', autospec=True, return_value='1234567\n')
    @mock.patch('skipper.runner.run', autospec=True)
    def test_run_with_defaults_from_config_file_including_volumes(self, skipper_runner_run_mock, *args):
        command = ['ls', '-l']
        run_params = command
        self._invoke_cli(
            defaults=config.load_defaults(),
        ...
        expected_fqdn_image = 'skipper-conf-build-container-image:skipper-conf-build-container-tag'
        skipper_runner_run_mock.assert_called_once_with(command, fqdn_image=expected_fqdn_image, environment=[],
                                                        interactive=False, net='host', volumes=['volume1', 'volume2'],
                                                        workdir=None, user=None)

    @mock.patch('__builtin__.open', create=True)
    @mock.patch('os.path.exists', autospec=True, return_value=True)
    @mock.patch('yaml.load', autospec=True, return_value=SKIPPER_CONF_WITH_WORKDIR)
    @mock.patch('subprocess.check_output', autospec=True, return_value='1234567\n')
    @mock.patch('skipper.runner.run', autospec=True)
    def test_run_with_defaults_from_config_file_including_workdir(self, skipper_runner_run_mock, *args):
        command = ['ls', '-l']
        run_params = command
        self._invoke_cli(
            defaults=config.load_defaults(),
        ...
        skipper_runner_run_mock.assert_called_once_with(command, fqdn_image=expected_fqdn_image, environment=[],
                                                        ...
                                                        workdir='test-workdir', user=None)

    @mock.patch('__builtin__.open', create=True)
    @mock.patch('os.path.exists', autospec=True, return_value=True)
    @mock.patch('yaml.load', autospec=True, return_value=SKIPPER_CONF_WITH_USER)
    @mock.patch('subprocess.check_output', autospec=True, return_value='1234567\n')
    @mock.patch('skipper.runner.run', autospec=True)
    def test_run_with_defaults_from_config_file_including_workdir(self, skipper_runner_run_mock, *args):
        command = ['ls', '-l']
    ...
    def test_shell(self, skipper_runner_run_mock, *args):
        ...
        skipper_runner_run_mock.assert_called_once_with(['bash'], fqdn_image=expected_fqdn_image, environment=[], interactive=True,
                                                        net='host', volumes=None, workdir=None, user=None)
