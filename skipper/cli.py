...
def _validate_port(matcher, port_forwarding):
    ...
    if not match:
        raise click.BadParameter(
            "Publish need to be in format port:port or port-port:port-port"
        )
...
def _validate_port_range(start, end):
    if start and end and end < start:
        raise click.BadParameter(
            f"Invalid port range: {start} should be bigger than {end}"
        )
...
@click.option(
    "--build-arg",
    multiple=True,
    help="Build arguments to pass to the container build",
    envvar="SKIPPER_BUILD_ARGS"
)
@click.option(
    "--build-context",
    multiple=True,
    help="Build contexts to pass to the container build",
)
@click.pass_context
def cli(
        ctx,
    ...
    ctx.obj["build_container_tag"] = (
        git.get_hash() if ctx.obj["git_revision"] else build_container_tag
    )
...
@click.option(
    "-c",
    "--cache",
    help="Use cache image",
    is_flag=True,
    default=False,
    envvar="SKIPPER_USE_CACHE_IMAGE",
)
@click.pass_context
def build(ctx, images_to_build, container_context, cache):
    ...
    tag = git.get_hash()
    build_args = (
        ctx.obj.get("build_args", ()) + (f"TAG={tag}",)
    )
    build_contexts = ctx.obj.get("build_contexts", ())
    main_context = container_context or ctx.obj.get("container_context")
    ...
    for image, dockerfile in valid_images_to_build.items():
        ...
        options = BuildOptions(
            ...
            main_context or os.path.dirname(dockerfile),
            build_contexts,
            build_args,
            cache,
        )
...
@click.option(
    "--force",
    help="Push image even if it's already in the registry",
    is_flag=True,
    default=False,
)
...
def _push(ctx, force, image, image_name, namespace, tag):
    ...
    if ret != 0:
        utils.logger.error(
            "Failed to tag image: %s as fqdn: %s", image_name, fqdn_image
        )
        sys.exit(ret)
    ...
    if tag in tags:
        if not force:
            utils.logger.info(
                "Image %s is already in registry %s, not pushing",
                fqdn_image,
                ctx.obj["registry"],
            )
        else:
            utils.logger.warning(
                "Image %s is already in registry %s, pushing anyway",
                fqdn_image,
                ctx.obj["registry"],
            )
...
@cli.command()
@click.option(
    "-r", "--remote", help="List also remote images", is_flag=True, default=False
)
@click.pass_context
    ...
    if remote:
        ...
        try:
            ...
            raise click.exceptions.ClickException(
                f"Got unknown error from remote registry {exp}"
            )

    print(
        tabulate.tabulate(
            images_info, headers=["REGISTRY", "IMAGE", "TAG"], tablefmt="grid"
        )
    )
...
@cli.command()
@click.option(
    "-r", "--remote", help="Delete image from registry", is_flag=True, default=False
)
@click.argument("image")
@click.argument("tag")
@click.pass_context
...
@click.option(
    ...
    is_flag=True,
    default=False,
    envvar="SKIPPER_INTERACTIVE",
)
@click.option("-n", "--name", help="Container name", default=None)
@click.option(
    "-e", "--env", multiple=True, help="Environment variables to pass the container"
)
@click.option(
    "-c",
    "--cache",
...
@click.option(
    "-p", "--publish", multiple=True, help="Publish a port", callback=_validate_publish
)
...
def _prepare_build_container(
    ...
    if image.tag:
        ...
        if image.registry and utils.remote_image_exist(
                image.registry, image.name, image.tag, username, password
        ):
        ...
        if not git_revision:
            raise click.exceptions.ClickException(
                f"Couldn't find build image {image.name} with tag {image.tag}"
            )
    else:
...
def _validate_project_image(image):
    ...
    if image not in project_images:
        raise click.BadParameter(
            f"'{image}' is not an image of this project, try {project_images}",
            param_hint="image",
        )
    ...
    if not images_to_build:
        ...
        for image in images_to_build:
            if image not in valid_images:
                utils.logger.warning(
                    "Image %s is not valid for this project! Skipping...", image
                )
                continue
            if not os.path.exists(valid_images[image]):
                utils.logger.warning(
                    "Dockerfile %s does not exist! Skipping...", valid_images[image]
                )
                continue
