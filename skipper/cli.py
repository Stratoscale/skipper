...
def _validate_port(matcher, port_forwarding):
    ...
    host_port_start_range, host_port_end_range, container_port_start_range, container_port_end_range = match.group(
        2, 4, 6, 8
    )
...
def cli(
    ...
    ctx.obj["namespace"] = ctx.default_map.get("push", {}).get("namespace")
...
def _push(ctx, force, image, image_name, namespace, tag):
    ...
    images_info = utils.get_remote_images_info(
        [repo_name], ctx.obj["registry"], ctx.obj.get("username"), ctx.obj.get("password")
    )
    tags = [info[-1] for info in images_info]
    if tag in tags:
        if not force:
            utils.logger.info("Image %s is already in registry %s, not pushing", fqdn_image, ctx.obj["registry"])
        else:
            utils.logger.warning("Image %s is already in registry %s, pushing anyway", fqdn_image, ctx.obj["registry"])
...
def images(ctx, remote):
    ...
    if remote:
        ...
        try:
            images_info += utils.get_remote_images_info(
                images_names, ctx.obj["registry"], ctx.obj.get("username"), ctx.obj.get("password")
            )
...
def rmi(ctx, remote, image, tag):
    ...
    if remote:
        ...
        utils.delete_image_from_registry(
            ctx.obj["registry"], image, tag, ctx.obj.get("username"), ctx.obj.get("password")
        )
    else:
...
def _expend_env(ctx, extra_env):
    environment = []
    # The configured push destination, so a Makefile or script running in the container can
    # target the same place as `skipper push` without repeating it. First in the list, so an
...
def _get_images_to_build(ctx, images_to_build):
    valid_images = ctx.obj.get("containers") or utils.get_images_from_dockerfiles()
    valid_images = {image: os.path.abspath(dockerfile) for image, dockerfile in valid_images.items()}
